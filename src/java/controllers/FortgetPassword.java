 ...
 * @author PhanSu
 */
@WebServlet(name = "FortgetPassword", urlPatterns = {"/forgetpass"})
public class FortgetPassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String confirmCode = (String) request.getParameter("confirmcode");
        ...
        if (!newPassword.equals(confirmPassword)) {
            ...
            if(confirmCode!=null){
                ...
                if(result.equals("true")){
                    Account account = new Account(newPassword, null, user);
                    //send http request to create new user
                    ObjectMapper mapper = new ObjectMapper();
                    String jsonRequest = mapper.writeValueAsString(account);
                    String result2 = APIUtils.sendPutRequest("http://localhost:8081/account/forgetpass/"+user.getEmail(), jsonRequest);
                    url = "/Views/Pages/User/Login.jsp";
                    System.out.print("Ket qua:" +result2);  
                } else{
                    request.setAttribute("confirmCodeError", "*Confirmed code does NOT match!");
                }                  
            }  
        }
